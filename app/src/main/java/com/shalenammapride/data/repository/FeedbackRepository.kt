...
class FeedbackRepository {
    ...
    fun getFeedback(): Flow<List<Feedback>> = callbackFlow {
    ...
    suspend fun addFeedback(feedback: Feedback): Result<Unit> = runCatching {
        val key = db.push().key ?: UUID.randomUUID().toString()
        db.child(key).setValue(feedback.copy(id = key)).await()
    }

    suspend fun deleteFeedback(id: String): Result<Unit> = runCatching {
